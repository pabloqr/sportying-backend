...
describe('WeatherService', () => {
  ...
  it('starts only one update request per geohash when data is missing', async () => {
    ...
    const updateSpy = jest
      .spyOn(service as any, 'updateWeather')
      .mockImplementation(
        () =>
          new Promise((resolve) => {
  ...
  it('gets weather by coordinates through the geohash flow', async () => {
    jest
      .spyOn(service, 'getWeatherFromGeohash')
      .mockResolvedValue({ alertLevel: 0 } as any);
  ...
  it('loads the complex coordinates before fetching weather by id', async () => {
    ...
    jest
      .spyOn(service, 'getWeatherFromCoordinates')
      .mockResolvedValue({ alertLevel: 0 } as any);
  ...
  it('runs purge and update on module init', async () => {
    const purgeSpy = jest
      .spyOn(service as any, 'purgeWeatherLogic')
      .mockResolvedValue(undefined);
    const updateSpy = jest
      .spyOn(service as any, 'updateWeatherLogic')
      .mockResolvedValue(undefined);
  ...
  it('wraps purge failures in InternalServerErrorException', async () => {
    ...
    await expect((service as any).purgeWeatherLogic()).rejects.toThrow(
